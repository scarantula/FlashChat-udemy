...
class ChatViewController: UIViewController {
    ...
    @IBAction func sendPressed(_ sender: UIButton) {
        ...
        if let message = messageTextfield.text, let sender = Auth.auth().currentUser?.email{
            db.collection(K.FStore.collectionName).addDocument(data: [K.FStore.senderField: sender, K.FStore.bodyField: message, K.FStore.dateField: Date().timeIntervalSince1970]) { error in
                if let e = error{
                    print("There was an issue while adding the data: \(e)")
                }
                else{
